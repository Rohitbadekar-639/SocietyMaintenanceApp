          ...
          {accounts.map((a) => (
            <article key={a.id} className="rounded-xl border border-slate-100 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-bold text-slate-950">{a.accountName} {a.primaryAccount && <span className="badge ml-2 bg-orange-50 text-orange-700">Primary</span>}</p>
                  <p className="mt-1 text-sm text-slate-600">{a.bankName}{a.branchName ? ` · ${a.branchName}` : ''}</p>
                  <p className="mt-2 text-sm"><span className="text-slate-500">A/C</span> {a.accountNumber}</p>
                  <p className="text-sm"><span className="text-slate-500">IFSC</span> {a.ifscCode}</p>
                  {a.upiId && <p className="text-sm"><span className="text-slate-500">UPI</span> {a.upiId}</p>}
                </div>
                <button type="button" className="text-sm font-semibold text-red-600" onClick={() => remove(a.id)}>Remove</button>
              </div>
            </article>
          ))}
